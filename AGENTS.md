...
3. Reads what community services actually have booked for that patient and
   ...
   (`ok`) - each carrying an urgency score for triage (`urgencyScore` in
   `reconcile()`'s result). The UI groups sweep results by status but doesn't
   currently sort or display the score itself - see `SweepResults` in
   `App.jsx` if wiring that up.
...
`POST /api/sweep` runs this same check across every patient with a filed
discharge summary, up to twelve concurrently (`checkSweep()` in
`dashboard/sweep.js`), streaming progress as newline-delimited JSON so the UI
can show a live count while it runs. It shares one hospital-documents fetch
and one community-view fetch across the whole batch rather than refetching
per patient - see the `communityResources()` note below for why that split
matters. Discharge decisions are cached for five minutes, keyed by team and
by the complete agent input (in-memory only, capped at 500 entries, cleared
on restart - see `createDecisionCache()` in `sweep.js`), so an unmodified
re-run doesn't repeat model calls it's already made; care-match verdicts
(whether a specific booking satisfies the decided care type) are always
evaluated fresh, since new bookings can appear between checks. Selecting a
flagged/review/matching patient from the sweep opens the same detail view a
single check would, without rerunning the model.
...
`model.js` still covers reconciliation for all 8 care types even though only
home-visit is actionable from the UI today - the agent needs the full
vocabulary to classify a note correctly. A booking flow for the other 7 types
is a reasonable future feature, not something missing.
...
| Tests | Node built-in test runner (`node --test`) | `dashboard/*.test.js`, `dashboard/src/model.test.js` |
...
- **Document what changes.** If you add a route, environment variable,
  heuristic, or user-visible behavior, update the root `README.md` and, if
  the call sequence or decision branches changed, `docs/flow.md` - see
  "Documentation" below.
...
- `dashboard/src/App.jsx` - React UI: connect to a team, run a full sweep or check one patient by ID, view the discharge summary/original note/decision, and - for a home-visit gap - review, edit, and confirm a booking.
...
- `dashboard/src/sweep.css` - sweep-page and sidebar-nav styling.
...
- `dashboard/server.js` - loopback-only Node static server, the read-only NHS-SIM proxy (`/api/connect`, `/api/check`, `/api/sweep`), and the one write route, `/api/book-home-visit`, gated on an explicit UI confirmation - it both schedules the visit and sends the patient a confirmation SMS as one operation.
- `dashboard/sweep.js` - the concurrent multi-patient sweep runner (`checkSweep`) and the team-isolated decision cache (`createDecisionCache`) it and `/api/check` both use.
- `dashboard/sweep.test.js` - worker-pool concurrency, decision-cache invalidation, and cache-eviction coverage.
- `dashboard/booking.test.js` - coverage for the `schedule_visit` + SMS write path, including retries and partial failure.
- `dashboard/discharges.test.js` - coverage for filed-summary selection and latest-by-`sentAt` ordering.
- `dashboard/careAgent.js` - the ADK agents that turn discharge-note sections into a structured care decision (with a plain-language summary and, when applicable, a draft home-visit booking) and turn a booking's details into a match verdict against the decided care type.
...
npm test          # Node tests: reconciliation, sweep, booking/SMS, discharges
```
...
- **`POST /api/sites/{site}/actions`** - the one write endpoint in the whole
  ...
  endpoint per action. Careloop makes exactly three calls through it, all
  from `scheduleHomeVisit()` in `server.js`, all firing only from the one
...
- **`communityResources()` fetches the whole community view once; `bookedCareFor()`
  is a pure filter over the result, not a fetch.** `handleSweep` calls
  `runCheck` once per patient over that one shared fetch for exactly this
  reason - without the split, each patient would re-fetch the same ~22KB
  view, turning a sweep over ~60 patients into ~60 identical requests.
  `handleCheck` only ever checks one patient, so a fetch-per-call would be
  harmless there too, but keep the split in both: don't inline the
  community-view fetch back into `bookedCareFor()` or `runCheck()`.
...
- `app.run(agent, promptString)` (the string shorthand) is what this tool
  ...
  need session state.
- The care-match agent (`evaluateCareMatch`) is deliberately scoped to less
  context than the decision agent: it sees only the decided care type, the
  decision's rationale, and each eligible booking's id/title/kind/status/data
  - never the raw discharge note. It also revalidates that the model
  returned exactly one verdict per input booking id, with no missing,
  duplicate, or extra ids, before trusting the result - the same
  boundary-check reasoning as the schema re-validation above.
...
- A booking or discharge time that's missing or unparseable is treated as
  *not provably before discharge*, not excluded - a missing timestamp isn't
  proof a booking predates discharge, so it's included in reconciliation
  rather than dropped. Same principle as the rest of this list: under-flag on
  missing data, don't over-flag on it. See `isAfterDischarge` in `model.js`.
...
- Run `npm test` after changing reconciliation, care-type matching, the
  sweep worker pool or decision cache, or the booking/SMS write path - the
  four suites (`model.test.js`, `sweep.test.js`, `booking.test.js`,
  `discharges.test.js`) cover each of those in turn.
- Add or update focused cases in `dashboard/src/model.test.js` for every
  ...
  needed, bookings before vs. after discharge, unverifiable care types, and
  mixed/ambiguous/missing match-verdict maps).
...
Update the root `README.md` when setup, environment variables, routes,
user-visible heuristics, privacy behavior, or operating limitations change.
Update [`docs/flow.md`](./docs/flow.md) whenever routes, call ordering, decision
branches, or retry/error behavior change - it's the authoritative call-
sequence reference and should stay in sync with `server.js`. Keep
instructions runnable from the repository root.
